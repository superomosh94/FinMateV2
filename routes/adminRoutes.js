...
router.post('/users/add', async (req, res) => {
  try {
    ...
    res.redirect('/admin/users/add?error=Failed to add user: ' + error.message);
  }
});

// Update the users/edit route in adminRoutes.js
router.get('/users/edit/:id', async (req, res) => {
  try {
    ...
    // Get user analytics data based on your actual database schema
...
router.post('/users/change-password/:id', async (req, res) => {
  try {
    ...
    res.redirect(`/admin/users/edit/${userId}?error=Failed to update password: ${error.message}`);
  }
});
...
// Add these API endpoints to your adminRoutes.js
...
router.get('/api/users/:id/statistics', async (req, res) => {
  try {
    ...
    // Get comprehensive statistics from your actual tables
  ...
  try {
    ...
    const [activities] = await db.pool.execute(`
      ...
      (SELECT 
        ...
        CONCAT('Budget: ', category) as description,
        amount,
      ...
      (SELECT 
        ...
        CONCAT('Savings Goal: ', name) as description,
...
router.get('/api/users/:id/activity', async (req, res) => {
  try {
    ...
    // Get user activity from multiple tables
    const [activities] = await db.pool.execute(`
      (SELECT 
      ...
      (SELECT 
        'password_changed' as activity_type,
        description,
        NULL as amount,
        created_at as activity_date,
        admin_id
       FROM user_activity WHERE user_id = ? AND activity_type = 'password_change')
...
// FIXED: Budget create route with correct column names according to your schema
...
// FIXED: Budget edit route with correct column names according to your schema
router.post('/budgets/edit/:id', async (req, res) => {
  try {
    ...
    res.redirect('/admin/budgets?error=Failed to update budget: ' + error.message);
  }
});
...
// Expense Management
// Expense Management - Updated route
router.get('/expenses', async (req, res) => {
  try {
    const { status } = req.query; // Get status filter from query params
    const currentFilter = status || ''; // Set current filter
...
// FIXED: Expense add route with correct column names according to your schema
router.post('/expenses/add', async (req, res) => {
  try {
    ...
    res.redirect('/admin/expenses/add?error=Failed to add expense: ' + error.message);
  }
});
...
// FIXED: Expense edit route with correct column names according to your schema
router.post('/expenses/edit/:id', async (req, res) => {
  try {
    ...
    res.redirect('/admin/expenses?error=Failed to update expense: ' + error.message);
  }
});
...
// Add these missing routes to your adminRoutes.js

// Teams Management (basic implementation)
...
router.get('/reports', async (req, res) => {
  try {
    ...
    console.log('✅ Reports data loaded successfully:', {
      expenses: expenseStats[0]?.total_expenses || 0,
      budgets: budgetStats[0]?.total_budgets || 0,
      users: userStats[0]?.total_users || 0,
      teams: teamStats[0]?.total_teams || 0
    });
...
router.get('/reports/export', async (req, res) => {
  try {
    ...
    // Get data for export (similar to main reports route)
...
// Notifications
// In your adminRoutes.js, update the notifications route
...
router.get('/teams/create', async (req, res) => {
  try {
    const [users] = await db.pool.execute('SELECT id, username FROM users WHERE is_active = 1 AND role_id IN (3, 4)'); // Team leaders and members
...
router.post('/teams/edit/:id', async (req, res) => {
  try {
    ...
    res.redirect('/admin/teams?error=Failed to update team');
  }
});
...
// Add this route to handle /profile redirects to /admin/profile
// Remove this conflicting redirect route:
// ======== ADMIN PROFILE ROUTES ========
router.get('/profile', async (req, res) => {
  try {
    console.log('Admin profile route accessed - User:', req.user);
...
router.get('/users/activity/:id', async (req, res) => {
  try {
    const userId = req.params.id;
    // Fetch user activity from your database
    const [activities] = await db.pool.execute(`
